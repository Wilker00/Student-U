/**
 * STUDENTU CORE ALGORITHMS
 * These algorithms drive the app's intelligence while keeping AI as a focused tool
 * for generation, not decision-making.
 */

// ============================================================================
// 1. KNOWLEDGE DEPENDENCY GRAPH
// Builds prerequisite relationships between concepts
// ============================================================================

function buildDependencyGraph(cards) {
    /**
     * Analyzes card content to detect which concepts depend on others.
     * Returns topologically sorted cards so prerequisites come first.
     */
    if (!cards || cards.length === 0) return [];
    
    const dependencies = {}; // cardId → [prerequisiteIds]
    const dependents = {}; // cardId → [dependentIds]
    ...
    // Keyword mappings for common concept relationships
    ...
    cards.forEach(card => {
        ...
        cards.forEach(other => {
            ...
            // Check if this card mentions the other concept
            if (cardKeywords.includes(otherTitle)) {
                addDependency(card.id, other.id);
            }
            
            // Check prerequisite patterns from the current concept toward prerequisite cards.
    ...
    // Topological sort to order cards
    const sorted = [];
    ...
    function visit(cardId) {
        ...
        if (visiting.has(cardId)) return; // Cycle detected
...
// Identifies which parts of a concept the student didn't engage with
// ============================================================================

function findComprehensionGaps(cardId, highlightedHTML, originalText) {
    /**
     * Analyzes highlighting patterns to identify comprehension gaps.
     * Returns segments that were NOT highlighted (potential knowledge gaps).
     */
    ...
    // Split original into sentences/segments
    ...
    sentences.forEach(sentence => {
        ...
        // If less than 30% of sentence is highlighted, flag it as a gap
    ...
    return {
        ...
        unhighlightedSegments: unhighlightedSegments.slice(0, 3), // Top 3 gaps
...
// Tracks if student is improving, plateauing, or declining
// ============================================================================

function calculatePerformanceVelocity(sessionHistory) {
    /**
     * Analyzes accuracy trend across recent sessions.
     * Returns velocity and adaptive recommendations.
     */
    ...
    const sumX = (n * (n - 1)) / 2; // Sum of zero-based indices
    ...
    if (velocity > 0.15) {
        ...
        multiplier = 1.4; // 40% harder next session
...
// Determines when to show a card again (SM-2 inspired but simplified)
// ============================================================================

function calculateOptimalReviewDate(card, performance, reviewHistory = []) {
    /**
     * Calculates scientifically-grounded review intervals.
     * Based on spaced repetition principles but with performance adaptation.
     */
    
    const baseIntervals = {
        'learned': 7,      // Show in 7 days
        'missed': 1,       // Show tomorrow
        'review': 3,       // Show in 3 days
        'neutral': 3       // Show in 3 days (not tested yet)
    };
    
    // Difficulty multiplier (advanced concepts need more spacing)
    const difficultyMultiplier = card.difficulty === 'Advanced' ? 0.7 : 
                                 card.difficulty === 'Beginner' ? 1.3 : 1.0;
    
    // Repetition factor (concepts reviewed many times can have longer intervals)
    const reviewCount = reviewHistory.length || 0;
    const repetitionFactor = Math.min(2.0, 1 + (reviewCount * 0.15));
    
    // Consistency bonus (if student got it right multiple times)
    const recentCorrect = reviewHistory.slice(-3).filter(r => r.correct).length || 0;
    const consistencyMultiplier = recentCorrect === 3 ? 1.5 : recentCorrect === 2 ? 1.2 : 1.0;
    
    let interval = baseIntervals[performance] || 3;
    interval = interval * difficultyMultiplier * repetitionFactor * consistencyMultiplier;
    
    // Minimum 1 day, maximum 60 days
    interval = Math.max(1, Math.min(60, Math.round(interval)));
    ...
    return {
        interval,
        reviewDate,
        dateString: reviewDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
        reasoning: `Base: ${baseIntervals[performance]} days × difficulty: ${difficultyMultiplier.toFixed(1)} × repetitions: ${repetitionFactor.toFixed(1)} × consistency: ${consistencyMultiplier.toFixed(1)}`
    };
}

// ============================================================================
// 5. WEAK SPOT CLUSTERING
// Groups related missed concepts for targeted drills
// ============================================================================

function clusterWeakSpots(missedCardIds, allCards) {
    /**
     * Groups related missed concepts by semantic similarity.
     * Returns clusters for focused drill-down sessions.
     */
...
// Adapts question difficulty based on real-time performance
// ============================================================================

function calibrateDifficulty(studentPerformanceData, historicalVelocity = null) {
    /**
     * Determines optimal difficulty for next session's questions.
     * Uses both current session performance and historical velocity.
     */
    ...
    // Incorporate velocity if available
    ...
    // Calculate difficulty mix for next session
    const difficultyMix = {
...
// Quality control: ensures AI questions match the material
// ============================================================================

function validateQuestionCoherence(card, question) {
    /**
     * Verifies that generated question actually relates to the card.
     * Prevents AI hallucinations/drift from original material.
     */
    ...
    const keywordMatches = cardKeywords.filter(kw => 
        questionKeywords.some(qk => 
            ...
            (kw.length > 6 && qk.length > 6 && 
             levenshteinDistance(kw, qk) <= 2)
        )
    );
    ...
    const isCoherent = overlapPercentage >= 40; // At least 40% keyword overlap
...
// ============================================================================
// UTILITY: Save and retrieve performance history
// ============================================================================

function getPerformanceHistory() {
    const stored = localStorage.getItem('studentu_performance_history');
    return stored ? JSON.parse(stored) : [];
}

function savePerformanceRecord(sessionData) {
    const history = getPerformanceHistory();
    history.push({
        timestamp: Date.now(),
        courseKey: sessionData.courseKey,
        correctAnswers: sessionData.correctAnswers || 0,
        missedAnswers: sessionData.missedAnswers || 0,
        cardStates: sessionData.cardStates || [],
        quizResults: sessionData.quizResults || [],
        duration: sessionData.duration || 0
    });
    localStorage.setItem('studentu_performance_history', JSON.stringify(history));
    return history;
}

function getPerformanceByDifficulty() {
    const history = getPerformanceHistory();
    const byDifficulty = {
