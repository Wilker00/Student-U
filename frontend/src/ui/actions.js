/**
 * Delegated UI actions — replaces inline onclick handlers.
 */
...
export function installActionDelegation(registry = {}) {
  Object.assign(registry, actionHandlers);

  document.addEventListener('click', event => {
    const target = event.target.closest('[data-action]');
    if (!target) return;
    const handler = registry[target.dataset.action];
    if (!handler) return;
